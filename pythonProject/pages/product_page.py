...
class ProductPage(BasePage):
    #метод проверяющий возможность добавления продукта в корзину
    def should_be_add_product_to_cart(self):
        add_to_cart = self.browser.find_element(*ProductPageLocators.ADD_PRODUCT_TO_CART)
        add_to_cart.click()

    # метод проверки, что название продукта добавленного в корзину совпадает с названием на странице продукта
    ...
    #метод проверки, что цена продукта добавленного в корзину совпадает с ценой на странице продукта
    ...
    #метод, что элемент не появляется в пустой корзине
    def should_not_be_success_message(self):
        assert self.is_not_element_present(*ProductPageLocators.SUCCESS_MESSAGE)
        "Success message is presented, but should not be"
    #метод проверки исчезает ли элемент со страницы
